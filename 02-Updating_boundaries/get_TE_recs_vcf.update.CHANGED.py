...
fout = open("DMEL_TE_Annotation.UPDATED.CHANGED.txt","w")
...
for line in TEfile:
    ...
    if TE_len > 750:
        ...
        for every_rec in vcf_recs[TE_chr]:
            ...
            if ( TE_start>(curr_vcfrec_start-1000) and TE_start<(curr_vcfrec_start+1000) ) and ( TE_end>(curr_vcfrec_end-1000) and TE_end<(curr_vcfrec_end+1000) ):
                ...
                if curr_SU > SU:   # Get the record with highest SU genrally thats the one that is proper and here minimum SU is 1000
        ...
        if changed==0 and num_of_recs>0:
            pass                                       
            #print "FLAG3 : There were more than 1 record found for this small TE but none of them met the condtion of SU > 1000"
    ...
    str_print = sup_fam + "\t" + TE_chr + "\t" + str(updated_TE_start) + "\t" + str(updated_TE_end) + "\t" + fam + "\t" + TE_col6 + "\t" + TE_col7 + "\t" + str(changed)
